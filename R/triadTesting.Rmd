...
response <- ask_gpt(model, temperature, prompts[1, ], api_key)
...
num_prompts <- ceiling(nrow(triad_data) / TRIADS_PER_PROMPT)
...
for (i in seq(1, num_prompts)) {
  ...
  # call to chatGPT
  response <- ask_gpt(model, temperature, prompts[i, ], api_key)
